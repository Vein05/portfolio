...
How research posters are built here. Companion to `design.md` (site figures) and `writing.md` (prose); the original approved spec is `research/2026-08-25-paper-poster-design.md`. First poster: `/poster/memory-targets` (EMNLP Findings 2026).
...
6. **Paper figures first, vector always.** Prefer the paper's own figures, converted from their source PDFs (`pdftocairo -svg <fig>.pdf public/poster/figures/<fig>.svg`) so they stay vector at any scale. Build custom SVG components only for table data that has no paper figure — computed from the raw numbers in Astro frontmatter (type the numbers once, derive coordinates), never hand-placed.
7. **Real numbers only.** Every value on the poster comes from the camera-ready paper. When the blog and the paper disagree (e.g. an earlier pilot audit vs the final one), the paper wins. Captions state the takeaway, then provenance ("Data: paper Table 5").
8. **Bento grid with a reading spine.** Layout is a 12-column grid of typed cells, but bento grids are ambiguous about order — so every content cell carries a numbered marker (①…⑧) forming an explicit reading path: problem → method → core result → consequences → validity → takeaway → TL;DR.
9. **Typography is calculated, not eyeballed.** At print scale 1pt = 0.353mm; readable distance ≈ text height in mm × 300. Tiers: title ~62pt (~6 m), cell titles 28pt (~2.5 m), body/TL;DR 22–23pt (~1.5 m, standard poster distance), captions/kickers 16pt (~1 m lean-in tier). Nothing below 14pt.
10. **End with a TL;DR band.** A full-width three-column strip (what we did / what we found / what you should do) is the last cell — the walk-away message, set at body size, never fine print.
...
2. Convert the paper's figure PDFs to SVG into `public/poster/figures/`.
